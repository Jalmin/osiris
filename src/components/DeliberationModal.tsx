        ...
        >
          ...
          >
            ...
            <div className="flex items-start justify-between gap-3 mb-3">
              {/* T24 contraste — tailles +2, fonds de cartes opaques via tokens */}
              <div className="flex items-center gap-1.5">
                <Hexagon className="w-3.5 h-3.5" style={{ color: 'var(--gold-primary)' }} />
                <span className="pythia-display text-[13px] tracking-[0.22em] text-[var(--gold-primary)]">MIROFISH SWARM</span>
                <span className="text-[10px] font-mono text-[var(--text-secondary)] tracking-widest">· DELIBERATION</span>
              </div>
            ...
            <div className="mb-3">
              ...
              <p className="text-[15px] text-[var(--text-primary)] leading-normal mt-1">{p.statement}</p>
            </div>

            {/* Consensus gauge */}
            <div className="flex items-center gap-3 rounded-xl border border-[var(--border-secondary)] p-3 mb-3" style={{ background: 'var(--bg-card)' }}>
              ...
              <div className="flex-1 min-w-0 flex flex-col gap-1.5">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[12px] font-mono font-bold" style={{ color: agreeColor }}>{agreement}</span>
                  <span className="text-[10px] font-mono text-[var(--text-secondary)]">· {spread}pt spread · {agents.length} voices</span>
                </div>
                {delta != null && (
                  <div className="text-[11px] font-mono text-[var(--text-secondary)] flex items-center gap-1">
                ...
                {p.reasoning && <p className="text-[11px] font-mono text-[var(--text-secondary)] leading-relaxed">{p.reasoning}</p>}
              </div>
            </div>
            ...
            {agents.length > 0 && (
              <div className="mb-3">
                <div className="text-[10px] font-mono tracking-widest text-[var(--text-secondary)] mb-2">WHERE THE COUNCIL LANDED</div>
            ...
            {agents.length > 0 ? (
              <div className="flex flex-col gap-2 mb-3">
                <div className="text-[10px] font-mono tracking-widest text-[var(--text-secondary)]">THE COUNCIL</div>
                {agents.map((a) => {
                  const m = AGENT_META[a.name] || { color: 'var(--text-secondary)', lens: '' };
                  const pct = Math.round(a.probability * 100);
                  return (
                    <div key={a.name} className="rounded-lg border border-[var(--border-secondary)] p-2" style={{ background: 'var(--bg-card)' }}>
                      <div className="flex items-center justify-between gap-2 mb-1">
                        <div className="flex items-center gap-1.5 min-w-0">
                          <Hexagon className="w-3 h-3 shrink-0" style={{ color: m.color }} />
                          <span className="text-[12px] font-bold" style={{ color: m.color }}>{a.name}</span>
                          <span className="text-[9px] font-mono text-[var(--text-secondary)] truncate">{m.lens}</span>
                          {a.model && (
                            <span className="text-[9px] font-mono shrink-0 px-1 py-[1px] rounded truncate max-w-[92px]"
                              title={`this vote was cast by ${a.model}`}
                              style={{ background: 'var(--chip-neutral-bg)', color: 'var(--text-secondary)', border: '1px solid var(--border-secondary)' }}>
                              {a.model}
                            </span>
                          )}
                        </div>
                        <span className="text-[13px] font-mono font-bold shrink-0" style={{ color: m.color }}>{pct}%</span>
                      </div>
              ...
              <div className="text-[11px] font-mono text-[var(--text-secondary)] text-center py-3 mb-2">
            ...
            {p.location && (
              <div className="flex items-center justify-between gap-2 pt-2 border-t border-[var(--border-secondary)]">
                <span className="text-[11px] font-mono flex items-center gap-1" style={{ color }}><MapPin className="w-3 h-3" /> {p.location}</span>
                {p.lat != null && p.lng != null && (
                  <button
                    ...
                    style={{ background: 'var(--chip-accent-bg)', color: 'var(--gold-primary)' }}
