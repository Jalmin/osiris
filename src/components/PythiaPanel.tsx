    ...
    >
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          {/* T24 contraste — tailles +2, fonds via tokens opaques */}
          <Eye className="w-3.5 h-3.5 text-[var(--gold-primary)]" />
          <span className="hud-text text-[14px] text-[var(--text-primary)]">PYTHIA</span>
          <span className="gotham-tag" style={{ fontSize: '9px', padding: '1px 5px', background: 'var(--chip-accent-bg)', color: 'var(--gold-primary)', borderRadius: 3 }}>ORACLE</span>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setShowScore((s) => !s)} title="Track record — Brier score, calibration, verdicts" className="flex items-center px-1.5 py-0.5 rounded" style={{ background: showScore ? 'var(--chip-accent-bg)' : 'var(--chip-neutral-bg)', color: showScore ? 'var(--gold-primary)' : 'var(--text-muted)' }}>
            <Target className="w-3 h-3" />
          </button>
          <button onClick={() => setShowSwarm((s) => !s)} title="Swarm models — pick a model for each persona" className="flex items-center px-1.5 py-0.5 rounded" style={{ background: showSwarm ? 'var(--chip-accent-bg)' : 'var(--chip-neutral-bg)', color: showSwarm ? 'var(--gold-primary)' : 'var(--text-muted)' }}>
            <Hexagon className="w-3 h-3" />
          </button>
          <button onClick={toggleLoop} title="Auto-refresh forecasts on an interval" className="flex items-center gap-1 text-[11px] font-mono px-1.5 py-0.5 rounded" style={{ background: snap.loop_enabled ? 'var(--chip-live-bg)' : 'var(--chip-neutral-bg)', color: snap.loop_enabled ? 'var(--cyan-primary)' : 'var(--text-muted)' }}>
            <Radio className="w-3 h-3" /> AUTO
          </button>
          <button onClick={predictNow} disabled={snap.generating} title="Forecast the world now" className="flex items-center gap-1 text-[11px] font-mono font-bold px-2 py-0.5 rounded disabled:opacity-50" style={{ background: 'var(--chip-accent-bg)', color: 'var(--gold-primary)' }}>
      ...
      {/* World-watch strip */}
      <div className="flex items-center justify-between text-[11px] font-mono mb-2 px-1 text-[var(--text-secondary)]">
      ...
      {score?.resolved ? (
        <div
          className="flex items-center justify-between text-[11px] font-mono mb-2 px-1 text-[var(--text-secondary)]"
      ...
      <div className={mobile ? 'flex flex-col gap-3' : 'overflow-y-auto flex flex-col gap-3 pr-1'}>
        {preds.length === 0 && (
          <div className="text-[12px] text-[var(--text-secondary)] py-6 text-center leading-relaxed">
            {snap.generating
          ...
          return (
            <div key={h.key}>
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <span className="text-[11px] font-mono tracking-widest" style={{ color: h.color }}>{h.label}</span>
              </div>
              {list.map((p) => (
                <div key={p.id}
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <span className="text-[13px] text-[var(--text-primary)] leading-normal">{p.statement}</span>
                    <span className="text-[13px] font-mono font-bold shrink-0 flex items-center gap-1" style={{ color: h.color }}>
                      {p.split && <span title="the swarm disagrees sharply" style={{ color: 'var(--alert-red)', fontSize: 10 }}>⚠</span>}
                  ...
                  {p.reasoning && <div className="text-[11px] font-mono text-[var(--text-secondary)] mt-1 leading-relaxed">{p.reasoning}</div>}
                  ...
                  {p.agents && p.agents.length > 0 && (
                    <div className="mt-1.5 text-[10px] font-mono flex items-center gap-1 text-[var(--text-secondary)]">
